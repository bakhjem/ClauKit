...
## 🎯 Marketing Kit — usage & real flows

Install with `ck init --kit marketing` (or `--kit both`). Adds the **`/mk:` namespace** — 50 marketing skills, 10 agents, 12 commands, plus WordPress publishing. Every `/mk:` command (except `/mk:plan`) **hard-fails without `plans/marketing-context.md`** — the hub holding your ICP, positioning, brand voice, competitors, goals, and channels. So step one is always the same.

> **The marketing rule**: `/mk:plan` once → it writes the context hub → every other `/mk:` command reads from it. Plan once, run many.

### 🧭 Marketing decision tree — which command do I need?
...
### Marketing Flow 2 — ✍️ Content → WordPress publish
...
### Marketing Flow 3 — 🔎 SEO — create & audit (incl. live WordPress posts)
...
### Marketing Flow 4 — 📈 Lead pipeline (B2B)
...
### 📋 Marketing quick reference — scenario → command

| Scenario | Command | Chain after |
|---|---|---|
| 🔧 First-time setup (ICP, voice) | `/mk:plan` | → any `/mk:` command |
...
### 🎯 Marketing patterns at a glance
