...
- `/fix --quick [issues]` - Quick bug fixes
- `/fix --review [issues]` - Complex problem solving with subagents
...
**Documentation** (dispatcher `/docs` + subcommand aliases):
- `/docs -init` (alias: `/docs init`) - Create initial documentation
- `/docs -update` (alias: `/docs update`) - Update existing documentation
- `/docs -summarize` (alias: `/docs summarize`) - Generate codebase summaries
...
- `/git merge [pr#|branch]` - Merge PR or branch (interactive strategy)
...
1. Run `/docs -update` (alias: `/docs update`)
