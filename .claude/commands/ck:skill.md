---
description: Skill operations dispatcher (flags -add -create -optimize -fix-logs)
argument-hint: -add [skill-name] [prompt] | -create [prompt-or-url] | -optimize [skill-name] [prompt] | -fix-logs [prompt-or-path]
---

## Variables

FLAG: $1 (one of `-add`, `-create`, `-optimize`, `-fix-logs`)
ARG1: $2
ARG2: $3
...
Dispatch to the matching operation based on {FLAG}.
...
- PROMPT: {ARG1}…{ARGN} (= `$ARGUMENTS` minus the flag)
...
- PROMPT: {ARG1}…{ARGN} (= `$ARGUMENTS` minus the flag) — optional path-to-skill or extra context
...
## Input Handling (shared across all flags)
...
## Examples
- `/ck:skill -add my-skill "add reference doc for X API"` — extend existing skill.
- `/ck:skill -create "create skill for parsing CSV with edge cases"` — new skill from prompt.
- `/ck:skill -create https://docs.example.com/api` — new skill from docs URL.
- `/ck:skill -optimize my-skill "reduce token usage"` — plan-first refactor.
- `/ck:skill -optimize` — optimize all skills (`*`).
- `/ck:skill -fix-logs` — diagnose & fix from `./logs.txt`.
