---
description: ⚡⚡⚡ Drive a feature spec → production through research, plan, code, test, review (full lifecycle)
argument-hint: [task hoặc plan path] [--fast|--auto|--from-plan|--no-test]
---
...
* Delegate to `planner` agent to create an implementation plan in `./plans/<YYMMDD-HHMM>-<slug>/plan.md`.
...
* Delegate to `code-reviewer` agent. Agent emits severity buckets: Critical / High / Medium / Low.
* **In `--auto` mode**: pass gate if `Critical = 0 AND High = 0`. Else fallback to user approval.
* **In default/--fast modes**: always require user approval after review.
* On critical/high findings: ask implementer agent to fix; re-run Stage 4 (Testing); re-review until clean.
...
- `/code` — legacy fast-path for "plan already exists". Equivalent to `/cook <plan> --from-plan`. Will be deprecated; prefer `/cook`.
- `/plan` — creates plan only; pair with `/cook plan.md` to execute.
- `/orchestrate` — multi-agent parallel fan-out (different concept from cook's sequential gated pipeline).
