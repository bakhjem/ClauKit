---
description: Documentation operations dispatcher (flags -init -update -summarize)
argument-hint: -init | -update [additional-requests] | -summarize [focused-topics] [should-scan-codebase]
---

## Variables

FLAG: $1 (one of `-init`, `-update`, `-summarize`)
ARG1: $2
ARG2: $3
...
- ADDITIONAL_REQUESTS: {ARG1}…{ARGN} (= `$ARGUMENTS` minus the flag) — optional extra instructions
